...
class RepaymentController extends Controller
{
    ...
    public function DueReport()
    {
     $page_title = 'Due Report';
        ...
        $orders = DB::table('orders')
                    ...
                    ->select('orders.*','customers.name as customer_name','customers.phone as customer_phone','order_instalments.id as order_installment_id')
                    ->get();
        //$installmentdue = Order::wherepayment_type(2)->latest()->get();
    ...
    public function submitInstalmentRepayment(Request $request)
    {
        ...
        if ($checkLast == 1) {
            ...
            $nextInstalment = InstalmentTime::whereNotIn('id', [$request->instalmentTime_id])->whereOrder_instalment_id($request->orderInstalment_id)->orderBy('id', 'DESC')->whereStatus(0)->first();
        ...
        $orderInstalment = OrderInstalment::where('order_id',$re['order_id'])
            ->select('grander_one_phone','grander_two_phone')
            ->first();
...
//        HelperProvider::send_sms($grander_two_phone, $message);
        ...
        session()->flash('message', 'Hey ,Boss ! Repayment Successfully Completed');
        session()->flash('type', 'success');
